...
router.post("/", async (req, res) => {
  ...
  if (!name) {
    res.status(404).json({ error: "O nome é obrigatorio!" });
    return;
  }
  if (!email) {
    res.status(404).json({ error: "Deve preencher o campo da email!"});
    return;
  }
  if (!telephone) {
    res.status(404).json({ error: "Deve telefone o campo da companhia!"});
    return;
  }
  if (!address) {
    res.status(404).json({ error: "Deve preencher o campo do endereço!"});
    return;
  }
  if (!company) {
    res.status(404).json({ error: "Deve preencher o campo da companhia!"});
    return;
  }
...
router.delete("/:id", async (req, res) => {
  ...
  if (!contact) {
    res.status(404).json({ message: "O contato nao foi encontrado!" });
    return;
  }
